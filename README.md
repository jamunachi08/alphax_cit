...
### 0.3.1 — flow board route fix
- The desk page moved from `/app/cit-flow` to **`/app/cit-flow`**. The old route
  was identical to the slug of the `CIT Process Flow` DocType, and Frappe's router resolves
  the DocType list first — so the board was unreachable and the list opened instead.
  `verify_tree.py` now fails on any Page route that collides with a DocType slug.
- The detailed board was showing `Flow Type: Journey`. Adding the field applied its default
  to the existing record; the seeder now owns the value explicitly on every migrate.
- Patch `v0_3_1_fix_flow_page_route` repairs sites already migrated: removes the shadowed
  page record, repoints the workspace shortcut and corrects both flow records.
...
`/app/cit-flow` opens on the **journey view**: one linear spine of **12 steps**
