...
def check_page_routes(doctypes):
	"""A Page whose route matches a DocType slug is unreachable.

	Frappe's router resolves /app/<slug> to the DocType list first, so the page
	silently never opens. This is only visible at runtime, never at import.
	"""
	slugs = {n.lower().replace(" ", "-") for n in doctypes}
	for root, _d, files in os.walk(APP):
		if os.sep + "page" + os.sep not in root + os.sep:
			continue
		for f in files:
			if not f.endswith(".json"):
				continue
			...
			if data.get("doctype") != "Page":
				continue
			route = data.get("page_name") or data.get("name")
			if route in slugs:
				ERRORS.append(
					f"Page '{route}' collides with the DocType of the same slug — "
					f"/app/{route} will open the list view, not the page")
			folder = os.path.basename(root)
			if folder != route.replace("-", "_"):
				WARNINGS.append(f"Page '{route}' sits in folder '{folder}'")
