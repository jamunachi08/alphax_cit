...
def get_flow(flow=None, with_counts=1):
	...
	for st in sorted(doc.stages, key=lambda s: s.sequence or 0):
		stages.append({
			...
			"colour": st.colour,
			"actor": st.get("actor"),
			"what_happens": st.get("what_happens"),
			"what_happens_ar": st.get("what_happens_ar"),
			"guarantee": st.get("guarantee"),
			"duration_hint": st.get("duration_hint"),
	...
	return {
		...
		"flow_type": doc.get("flow_type") or "Detailed",
		"description": doc.description,
		"stages": stages,
		"available_flows": frappe.get_all(
			"CIT Process Flow", filters={"disabled": 0},
			fields=["name", "flow_name", "flow_type"], order_by="flow_type asc"),
	}
