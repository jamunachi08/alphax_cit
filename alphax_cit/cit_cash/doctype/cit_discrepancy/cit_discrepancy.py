...
class CITDiscrepancy(Document):
	...
	def post_journal_entry(self):
		...
		cash_account = None
		vault = None
		if self.counting_session:
			vault = frappe.db.get_value("Counting Session", self.counting_session, "vault")
		if not vault and self.cit_trip:
			vault = frappe.db.get_value("CIT Trip", self.cit_trip, "vault")
		if not vault:
			vault = st.default_vault or frappe.db.get_value(
				"CIT Vault", {"company": self.company, "disabled": 0}, "name")
		if vault:
			cash_account = frappe.db.get_value("CIT Vault", vault, "cash_account")
